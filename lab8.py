...
def tryToConnect():
    ...
    if g_bConnected:
        printToMessages("You're already connected")
    else:
        ...
        try: 
            ...
            g_bConnected = True
            printToMessages("succesfully connected")
            g_app.connectButton["text"] = "disconnect"
            g_app.connectButton["bg"] = "red"
        except socket.error as error:
            print("connection error: {}".format(error))
            message = myAddrFormat(ipAddr)
            printToMessages("could not connect to {}".format(message))
